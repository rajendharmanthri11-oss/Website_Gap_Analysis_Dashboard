...
radar_col, bar_col = st.columns([3, 2])

with radar_col:
    cats = ["Visitor Volume", "Lead Rate", "Conv Rate (L→C)", "Overall CR", "Lead Volume"]
    ...
    v_vol  = norm([TOTALS[d]["visitors"]  for d in DOMAIN_LIST])
    lr     = norm([TOTALS[d]["lead_rate"] for d in DOMAIN_LIST])
    cr     = norm([TOTALS[d]["conv_rate"] for d in DOMAIN_LIST])
    ocr    = norm([TOTALS[d]["overall_cr"]for d in DOMAIN_LIST])
    l_vol  = norm([TOTALS[d]["leads"]     for d in DOMAIN_LIST])

    radar_fig = go.Figure()
    for i, domain in enumerate(DOMAIN_LIST):
        vals  = [v_vol[i], lr[i], cr[i], ocr[i], l_vol[i]]
        ...
        radar_fig.add_trace(go.Scatterpolar(
            ...
            fill="toself",
            name=domain,
            line=dict(color=DOMAINS[domain]["color"], width=2),
            fillcolor=DOMAINS[domain]["rgba_low"],
        ))
    radar_fig.update_layout(
        polar=dict(
            bgcolor="#1a1f2e",
            radialaxis=dict(visible=True, range=[0, 1], color="#4b5563", tickfont=dict(size=9)),
            angularaxis=dict(color="#94a3b8"),
        ),
        paper_bgcolor="#1a1f2e",
        font=dict(color="#e2e8f0"),
        legend=dict(bgcolor="#1a1f2e", font=dict(color="#e2e8f0")),
        title=dict(text="Normalised Performance Radar", font=dict(color="#e2e8f0", size=13)),
        height=370,
